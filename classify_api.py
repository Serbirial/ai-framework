...
def classify_likes_dislikes():
    ...
    classification = pre_processing.classify_likes_dislikes_user_input(model, None, user_input, likes, dislikes)
...
def classify_social_tone():
    ...
    classification = pre_processing.classify_social_tone(model, None, user_input)
    return jsonify({"classification": classification})
...
def determine_moods():
    ...
    moods = pre_processing.determine_moods_from_social_classification(classification, top_n=top_n)
...
def classify_moods_into_sentence():
    ...
    mood_sentence = pre_processing.classify_moods_into_sentence(model, None, moods_dict)
...
def classify_user_input():
    data = request.get_json()
    user_input = data.get('user_input', '')
    category = pre_processing.classify_user_input(model, None, user_input)
...
def extract_search_query():
    ...
    query = pre_processing.extract_search_query_llama(model, input_text, role=role)
...
def classify_summarize_input():
    ...
    summary = pre_processing.classify_summarize_input(model, input_text, max_tokens=max_tokens)
